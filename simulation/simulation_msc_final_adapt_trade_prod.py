...
output_folder = f'./output/adapt_trade_prod/{scenario}/'     # folder to write results to
...
for t in range(0,tau):
    ...
    if t >= prerun and crop_flag:
        ...
        # Production adaptation for maize and rice
        if t >= prerun + 10:
            maize_indices = is_maize.nonzero()[0]
            for idx in maize_indices:
                output[idx, 0] = output[idx, 0] * (1. + min((t-prerun-10)*0.01, 0.15))
            rice_indices = is_rice.nonzero()[0]
            for idx in rice_indices:
                output[idx, 0] = output[idx, 0] * (1. + (t-prerun-10)*0.005)
